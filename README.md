...
```bash
yarn add xicons # or npm install xicons --save
```
...
```ts
import { AlertOutline } from 'xicons';
...
## Render Helpers
```ts
import { AccountBookFill } from './lib/dist';
import { renderIconDefinitionToSVGElement } from './lib/icons/lib/helpers';
